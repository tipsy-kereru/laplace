...
Laplace는 에이전트 루프가 안전하게 할 수 있는 일의 범위를 넓히지만, 모든 루프가 가지는 근본적 한계까지 없애지는 못합니다. 실무에서 중요한 세 가지가 있습니다.

1. **모델이 의도를 오독할 수 있습니다.** 모호한 프롬프트는 한쪽 방향으로 해석된 채 진행될 수 있습니다. AGENTS.md의 "추측하지 말고 멈추기" 규칙, 타입별 증거 게이트(SPEC-003), 그리고 draft → approved의 사람 승인 게이트가 이를 줄여주지만 완전히 제거하진 못합니다. 범위 결정이 의심스러우면 멈추고 명확히 하세요.
2. **자율성이 넓어질수록 오류의 폭발 반경이 커집니다.** 루프의 자유도가 넓을수록 단 하나의 잘못된 결정이 미치는 영향이 커집니다. 프로덕션 브랜치에서 `freerange all`은 무인으로 나쁜 커밋을 게시할 수 있고, feature 브랜치에서 `freerange flow`는 검토 가능한 결과물을 만듭니다. 스코프와 TTL은 이것을 묶어두기 위해 존재합니다 — 사용하세요. `docs/freerange-recipes.kr.md`를 참고.
3. **최종 검증자는 사람입니다.** 어떤 훅도 판단을 대체하지 못합니다. 루프는 *검토 가능한* 결과물(증거, 리포트, diff)을 만들 뿐, *신뢰할 수 있는* 결과물을 만들진 않습니다. 리포트를 읽고, diff를 검토하고, 그 다음 승인하세요. 하드 안전 바닥이 치명적 케이스를 막아줍니다; 나머지는 당신의 몫입니다.

이 한계들은 설계의 대가입니다. 한계 없는 루프는 검토 없는 루프이고, 검토 없는 자율은 Laplace가 막기 위해 존재하는 실패 모드 그 자체입니다.
